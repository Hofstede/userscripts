/*
  ...
  May NOT delete anything if:
  - Twitter changes its front-end code
  - Twitter doesn't send or process some of the "delete" requests.
    (A simple test resulted in a bit less than half of the tweets
    being deleted in a run of 200 tweets.)
  - Twitter caps your browser or Twitter account after a number
    of requests. (Highly likely. I need to research this.)

  If you want to delete tens of thousands of tweets, best-case
  scenario is that it'll take days of running this script in a
  background tab, with manual intervention to relaunch it once
  in a while. And I said BEST-CASE SCENARIO.
  ...
  - The first batch of options should have sensible values, but you
    may want to change them for different behavior.
    The KEEP_LATEST and KEEP_RECENT options will be combined. If a
    tweet meets any of the two criteria, it will be kept.

  - You may want to tweak the waiting times.
    Generally speaking, higher times mean less skipped tweets.

  - The one time setting I found was critical is WAIT_AFTER_CONFIRM.
    After you ask for a tweet's deletion, the twitter.com UI is busy with
    showing a confirmation message (and possibly getting the information
    for that message with a HTTP request), and if you process the next
    tweet too soon you won’t be able to confirm deletion for it.
